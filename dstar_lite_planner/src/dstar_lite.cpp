...
{
    for (int i = 0; i < static_cast<int>(field_ptr->GetHeight()); ++i) {
        std::vector<DstarNode> line;
        for (int j = 0; j < static_cast<int>(field_ptr->GetWidth()); ++j)
...
Key DstarLite::CalculateKey(DstarNode* node) {
    ...
    float k1 = min_g_rhs + heuristic_function_(start_pos_, {node->x, node->y}) + km;
    float k2 = min_g_rhs;
    return {k1, k2};
}
    ...
    while (!open_.empty() && (open_.begin()->first < CalculateKey(start_node) ||
        ...
        if (k_old < k_new) {
            ...
            for (auto n : field_ptr_->GetNeighbours(u->x, u->y))
                UpdateVertex(&grid_[n.first][n.second]);
        } else {
...
std::optional<Coordinates> DstarLite::GetNextNode() {
    auto neighbours = field_ptr_->GetNeighbours(start_pos_.first, start_pos_.second);
    if (neighbours.empty()) return std::nullopt;
    ...
    if (min_cost == std::numeric_limits<float>::infinity()) return std::nullopt;
...
void DstarLite::UpdateObstacles(const std::vector<std::pair<Coordinates, char>>& updates) {
    if (updates.empty()) return;

    const int h = static_cast<int>(field_ptr_->GetHeight());
    const int w = static_cast<int>(field_ptr_->GetWidth());
    std::unordered_set<int> marked;
    std::vector<Coordinates> affected;

    const std::array<Coordinates, 5> deltas = {
        Coordinates{0,0},{1,0},{0,1},{-1,0},{0,-1}};

    for (const auto& [pos, value] : updates) {
        if (field_ptr_->Get(pos.first, pos.second) == value) continue;
        field_ptr_->Set(pos.first, pos.second, value);
        for (const auto& [dx, dy] : deltas) {
            int nx = pos.first + dx, ny = pos.second + dy;
            if (nx < 0 || nx >= h || ny < 0 || ny >= w) continue;
            if (marked.insert(nx * w + ny).second) affected.push_back({nx, ny});
        }
    }

    for (const auto& p : affected)
        UpdateVertex(&grid_[p.first][p.second]);
}
