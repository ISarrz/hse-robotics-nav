...
MPPIPredict::MPPIPredict(MPPIPredictConfig config, unsigned int seed)
    : MPPI(config, seed), pcfg_(config) {}

std::optional<std::pair<int, int>> MPPIPredict::GetNextNode(
    ...
    for (int k = 0; k < N; ++k) {
        ...
        for (int t = 0; t < T; ++t) {
            ...
            for (const auto& o : obstacles) {
                ...
                if (man <= 1) {
                    cost += pcfg_.predict_weight * cfg_.obstacle_cost
